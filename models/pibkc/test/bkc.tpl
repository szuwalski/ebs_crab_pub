DATA_SECTION
 ...
 !! ad_comm::change_datafile_name("catch_dat.DAT");
  ...
  //init_vector wt_at_size(1,size_n)
   ...
   //!!cout<<"wt_at_size"<<wt_at_size<<endl;
...
PARAMETER_SECTION
  ...
  init_bounded_dev_vector rec_devs(styr,endyr,-20,20,1)
  ...
  init_bounded_number fish_tot_sel_50(25,150,-1)
  init_bounded_number fish_tot_sel_slope(0.0001,20,-1) 
  ...
  vector pred_retained_bio(styr,endyr)
...
FUNCTION evaluate_the_objective_function
  ...
  for (int year=styr;year<=endyr;year++)
   for (int size=1;size<=size_n;size++)
   {
	...
	//pred_retained_bio(year) += pred_retained_size_comp(year,size)*wt_at_size(size)/1000;
  ...
  ret_cat_like = 0;
  for (int year=1;year<=ret_cat_yr_n;year++)
    ret_cat_like += square( log(pred_tot_n(ret_cat_yrs(year))) - log(ret_cat_numbers(year))) / (2.0 * square(sigma_numbers_ret));
 ...
 //penalties on m 
  ...
  cout<<ret_cat_like<< " " << ret_comp_like << " " << tot_comp_like << " " <<endl;
...
REPORT_SECTION
 ...
 report <<"$pred_retained_bio" << endl;
  for(int i=styr; i<endyr; i++)
  {
    report << (pred_retained_bio(i))<<endl;
  }
