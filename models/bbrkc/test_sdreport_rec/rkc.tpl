...
// initial year numbers at size and selectivity
 ...
 for(int year=styr;year<endyr;year++)
  ...
  for (int size=1;size<=size_n;size++) 
	temp_n(size) = n_size_pred(year,size) * exp(-1*(0.17)*exp( nat_m(year,size)));
	  ...
	  // growth
       ...
       temp_n(1) += recruits(year)*temp_prop_rec(1);
       temp_n(2) += recruits(year)*temp_prop_rec(2);
	   temp_n(3) += recruits(year)*temp_prop_rec(3);
  ...
  recruits(endyr) = exp(log_avg_rec + rec_devs(endyr));
...
FUNCTION evaluate_the_objective_function
  ...
  for (int year=styr;year<=endyr;year++)
   for (int size=1;size<=size_n;size++)
    if (year!=2020)
     surv_sc_like += sc_eff_samp*(n_size_obs(year,size)/sum_numbers_obs(year)) * log(0.00001 + (selectivity(year,size)*n_size_pred(year,size)/numbers_pred(year)) / (0.000001 + n_size_obs(year,size)/sum_numbers_obs(year)));
  surv_sc_like = -1*surv_sc_like;
