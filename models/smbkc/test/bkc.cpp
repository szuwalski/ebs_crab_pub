 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  molt_sel_slope.allocate(0.01,20,est_molt,"molt_sel_slope");
...
void model_parameters::evaluate_the_objective_function(void)
{
  ...
  for (int year=styr;year<=endyr;year++)
   if (year!=2020)
    num_like += square( log(numbers_pred(year)+0.01) - log(n_obs(year))) / (2.0 * square(sigma_numbers(year)));
   // likelihoods
  ret_cat_like = 0;
  for (int year=1;year<=ret_cat_yr_n;year++)
    ret_cat_like += square( log(pred_retained_n(ret_cat_yrs(year))+0.01) - log(ret_cat_numbers(year))) / (2.0 * square(sigma_numbers_ret));
  tot_cat_like = 0;
  for (int year=1;year<=tot_cat_yr_n;year++)
    tot_cat_like += square( log(pred_tot_n(tot_cat_yrs(year))+0.01) - log(tot_cat_numbers(year))) / (2.0 * square(sigma_numbers_tot));
 ...
 //penalties on m 
  ...
  f_prior.initialize();
   // for (int year=1;year<=ret_cat_yr_n;year++)
      // f_prior += pow((exp(log_f+f_dev(year))-.51)/ (sqrt(2)*sqrt(1)),2.0);
  if(est_m_devs>0)
  {
  nat_m_like =0;
  for (int year=styr;year<=endyr;year++)
