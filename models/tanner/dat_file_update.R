...
sizes<-seq(27.5,182.5,5)
styr<-1975
endyr<-2025
years<-seq(styr,endyr)

channel <- "API"
specimen_data <- crabpack::get_specimen_data(species = "TANNER",
                                             region = "EBS",
                                             years = c(styr:endyr),
                                             channel = channel)
...
rownames(n_at_size_mat)<-n_at_size_new_wide[,1]
#==get index of mature and immature animals
#==add zeros
yrs<-n_at_size_new_wide[,1]
allyr<-seq(min(yrs),max(yrs))
use_imm_n<-matrix(0,ncol=length(sizes),nrow=length(allyr))
use_mat_n<-matrix(0,ncol=length(sizes),nrow=length(allyr))
rownames(use_imm_n)<-allyr
rownames(use_mat_n)<-allyr

for(x in 1:length(allyr))
{
 use_imm_n[x,]  <- unlist(n_at_size_imm[(match(allyr[x],as.numeric(rownames(n_at_size_imm)))),-c(ncol(n_at_size_imm))])
 use_mat_n[x,]  <- unlist(n_at_size_mat[(match(allyr[x],as.numeric(rownames(n_at_size_mat)))),-c(ncol(n_at_size_mat))] )
}
use_imm_n[is.na(use_imm_n)]<-0
use_mat_n[is.na(use_mat_n)]<-0
...
size_trans<-read.csv("models/tanner/test/size_trans_tanner.csv",header=F)
...
# Specify output file
outfile <- "models/tanner/test/tanner.dat"
# Create an empty file (overwrite if exists)
file.create(outfile)

# Write header and values
cat("# eastern Bering Sea Tanner crab pop dy model\n", file = outfile)
...
cat(rownames(use_mat_n), file = outfile, append = TRUE,"\n")
...
in_p_mat<-out_p_mat[-1,]
avg_p<-matrix(out_p_mat[1,],nrow=1)
...
emp_sel<-unlist(read.csv("models/Tanner/test/empirical_selectivity.csv",header=F))
...
tmp<-filter(calc_ind,CATEGORY=='small_male')%>%
  group_by(YEAR)%>%
  summarize(cv_mu=mean(ABUNDANCE_CV))

yrs<-tmp$YEAR
allyr<-seq(min(yrs),max(yrs))
imm_cv<-rep(0,length=length(allyr))
names(imm_cv)<-allyr

for(x in 1:length(allyr))
  imm_cv[x]  <- unlist(tmp[(match(allyr[x],yrs)),2])
imm_cv[is.na(imm_cv)]<-10

cat(imm_cv, file = outfile, append = TRUE,"\n")

cat("# CV mature numbers", file = outfile, append = TRUE,"\n")
tmp<-filter(calc_ind,CATEGORY=='legal_male')%>%
  group_by(YEAR)%>%
  summarize(cv_mu=mean(ABUNDANCE_CV))
...
cat(c(0.01,0.05,0.03), file = outfile, append = TRUE,"\n")
...
cat(c(0.001,0.001), file = outfile, append = TRUE,"\n")
...
cat(0.0001, file = outfile, append = TRUE,"\n")
...
# Specify output file
outfile <- "models/tanner/test/tanner.pin"
file.create(outfile)

# Write header and values
cat("# eastern Bering Sea Tanner pop dy model .pin file\n", file = outfile)
cat(paste("# Generated on: ", Sys.Date()),"\n", file = outfile, append = TRUE)

cat("# log_n_imm", file = outfile, append = TRUE,"\n")
cat(log(rep(mean(use_imm_n[1,]),ncol(use_imm_n))), file = outfile, append = TRUE,"\n")
cat("# log_n_mat", file = outfile, append = TRUE,"\n")
cat(log(rep(mean(use_mat_n[1,]),ncol(use_imm_n))), file = outfile, append = TRUE,"\n")
