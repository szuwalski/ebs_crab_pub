...
rep_files<-"/models/snow/test/snow_down.rep"
...
#==immature indices
div_n<-c(max(outs_in[[1]]$imm_n_obs),max(outs_in[[1]]$imm_n_obs))
ind_dat_imm<-NULL
...
#==mature indices
div_n<-c(max(outs_in[[1]]$mat_n_obs),max(outs_in[[1]]$mat_n_obs))
ind_dat_mat<-NULL
for(x in 1:length(outs_in))
{
  years<-seq(outs_in[[x]]$styr,outs_in[[x]]$endyr)
